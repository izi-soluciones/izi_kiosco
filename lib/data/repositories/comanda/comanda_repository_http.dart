...
class ComandaRepositoryHttp extends ComandaRepository {
  ...
  @override
  Future<List<ConsumptionPoint>> getConsumptionPoints(int sucursal, int contribuyente,{String? roomId}) async {
    ...
    if (response.statusCode == 200) {
      (response.data as List).removeWhere((element) => element["eliminado"]==true);
  ...
  @override
  Future<void> emitContingencia({required InvoiceDto invoice, required int orderId}) async{
  ...
  @override
  Future<Payment> addPayment({required Payment payment, required int orderId,required int contribuyente})async {
    try{
  ...
  @override
  Future<Charge> generateQr({required int contribuyenteId, required QrDto qr}) async{
    try{
  ...
  @override
  Future<void> freeConsumptionPoint({required String id,required int sucursal,required int contribuyente})async {
    try{
  ...
  Future<void> cancelOrder({required int orderId}) async{
    try{
      ...
      var response = await _dioClient.put(
          uri: path,
          options: Options(responseType: ResponseType.json), body: {});
      if (response.statusCode != 200) {
        if(response.data?["status"] ?? false){
          throw response.data?["data"];
        }
  ...
  @override
  Future<void> markInternal({required InternalMovementDto internalMovementDto}) async{
    try{
      String path = "/comandas/${internalMovementDto.comandaId}/consumo-interno";
      var response = await _dioClient.put(
          uri: path,
          options: Options(responseType: ResponseType.json), body: internalMovementDto.toJson());
      if (response.statusCode != 200) {
        if(response.data?["status"] ?? false){
          throw response.data?["data"];
        }
  ...
  Future<List<CategoryOrder>> getCategories({required int sucursal, required int contribuyente}) async{
    try{
      ...
      String path = "/contribuyentes/$contribuyente/sucursales/$sucursal/inventario/recetas/categorias";
      var response = await _dioClient.get(
          uri: path,
          options: Options(responseType: ResponseType.json));
      if (response.statusCode == 200) {
  ...
  Future<Comanda> emitOrder({required NewOrderDto newOrder}) async{
    try{
      ...
      var response = await _dioClient.post(
          uri: path,
          options: Options(responseType: ResponseType.json), body: newOrder.toJson());
      if (response.statusCode == 200) {
        return Comanda.fromJson(response.data);
      } else {
        throw response.data;
  ...
  Future<Comanda> editOrder({required NewOrderDto newOrder}) async{
    try{
      ...
      var response = await _dioClient.put(
          uri: path,
          options: Options(responseType: ResponseType.json), body: newOrder.toJsonEdit());
      if (response.statusCode == 200) {
        if((response.data is Map) && (response.data as Map)["comanda"]!=null){
