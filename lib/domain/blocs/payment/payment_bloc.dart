...
class PaymentBloc extends Cubit<PaymentState> {
  ...
  selectPayment(PaymentType paymentType,AuthState authState) async {
    try{
      if(paymentType==PaymentType.cashRegister){
        ...
        Comanda comanda = await _comandaRepository.markAsCreated(state.paymentObj?.id ?? 0);
  ...
  Future<bool> _makeCardRetailPayment(AuthState authState,{bool atc=false,bool linkser=false,bool contactless=true}) async{
    ...
    try{
      ...
      if(linkser){
        cardPayment = await _comandaRepository.callCardPayment(amount: _getIntFromDecimal(_roundToNDecimals(state.paymentObj?.amount ?? 0, 2)), ip: authState.currentDevice!.config.ipLinkser!);
      }
      else{
        try{cardPayment=await _comandaRepository.callCardPaymentATC(amount: (state.paymentObj?.amount ?? 0).moneyFormat(),ip: authState.currentDevice!.config.ipAtc!,contactless: contactless);
      ...
      for(var i=0;i<10;i++){
        try{
          await _comandaRepository.markPaymentATC(charge.token??'', state.paymentObj?.uuid??"",charge.intentoPago);
          success =true;
          break;
        }
        catch(e){
  ...
  Future<bool> _makeCardOrderPayment(AuthState authState,{bool atc=false,bool linkser=false,bool contactless=true}) async{
    ...
    try{
      ...
      for(var i=0;i<10;i++){
        try{
          await _comandaRepository.markPaymentATC(charge.token??'', charge.uuid,null);
          success =true;
          break;
        }
        catch(e){
  ...
  Future<bool> makeCardPayment(AuthState authState,{bool atc=false,bool linkser=false,bool contactless=true})async{
    if(_validateInputs() && (atc || linkser) && state.paymentObj?.isComanda == true){
      return await _makeCardOrderPayment(authState,atc: atc,contactless: contactless,linkser: linkser);
    }
    else if(_validateInputs() && (atc || linkser) && state.paymentObj?.isComanda == false){
      return await _makeCardRetailPayment(authState,atc: atc,contactless: contactless,linkser: linkser);
    }
    return false;
  ...
  Future<bool> generateQR(AuthState authState) async {
    try{
      ...
      log(e.toString());
      emit(state.copyWith(qrLoading: false,qrCharge: ()=>null,status: PaymentStatus.cardError));
      emit(state.copyWith(status: PaymentStatus.successGet));
      return false;
    }
  }
  ...
  _printRolloOrder(AuthState authState,{required int orderNumber,int? customOrderNumber})async{
    var tmp = await PrintTemplate.order80(orderNumber,customOrderNumber,authState.currentContribuyente!, authState.currentSucursal!,state.paymentObj);
